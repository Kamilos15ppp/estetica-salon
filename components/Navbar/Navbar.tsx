...
const Navbar = () => {
  ...
  const menuList = Hrefs.map(({ path, text }: Href) => (
    <li key={path} onClick={isMobileView ? toggleMenu : undefined}>
  ...
  return (
    <header className={styles.container}>
      {isMobileView && (
        <div
          className={`${styles.menuIcon} ${
            isMenuToggled ? styles.menuIconRotate : ''
          }`}
          onClick={toggleMenu}
        >
          <BsList />
      ...
      <nav
        className={`${styles.navbar} ${isMenuToggled ? styles.toggleOn : ''}`}
      >
        <ul>{menuList}</ul>
      </nav>
    </header>
